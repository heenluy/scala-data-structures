...
class ArrayBasedStack[T: ClassTag] private (
    ...
    */
    ...
    def push(item: T): ArrayBasedStack[T] =
    ...
    */
    ...
    def pop(): Option[(T, ArrayBasedStack[T])] =
        if size > 0 then
...
object ArrayBasedStack:
    def empty[T: ClassTag]: ArrayBasedStack[T] =
        new ArrayBasedStack[T](new Array[T](0), 0)
    
    def apply[T: ClassTag](items: T*): ArrayBasedStack[T] =
        new ArrayBasedStack[T](items.toArray, items.length)
