...
class ListBasedStack[A] private (private var elements: List[A]) {
    ...
    */
    def pop(): Option[(A, ListBasedStack[A])] = elements match
        case Nil => None
        case head :: tail =>
...
object ListBasedStack {
    ...
    */
    def empty[A]: ListBasedStack[A] = new ListBasedStack[A](Nil)

    /*
        * Esse pode criar uma pilha com vários elementos.
        * Exemplo: Stack.apply[Int](1, 2, 3)
    */
    def apply[A](items: A*) = new ListBasedStack[A](items.toList)
}